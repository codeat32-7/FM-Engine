  ...
  const handleSetup = async () => {
    ...
    try {
      ...
      const { error: profErr } = await supabase.from('profiles').upsert({
        ...
        full_name: adminName,
        role: 'admin',
        onboarded: true // explicitly set in DB
      }, { onConflict: 'id' });
...
const App: React.FC = () => {
  ...
  const syncOrphanedRequesters = useCallback(async () => {
    if (!currentUser?.org_id) return;
    const { data: allSRs } = await supabase.from('service_requests').select('requester_phone').eq('org_id', currentUser.org_id);
    if (!allSRs) return;

    const srPhones = [...new Set(allSRs.map(sr => sr.requester_phone).filter(Boolean))];
    const { data: existingProfiles } = await supabase.from('profiles').select('phone').in('phone', srPhones);
    const existingPhones = new Set(existingProfiles?.map(p => p.phone) || []);

    const orphans = srPhones.filter(p => p && !existingPhones.has(p));
    if (orphans.length > 0) {
  ...
  // Fix: Missing handleApproveTenant implementation to handle approval logic
  const handleApproveTenant = async (requester: Requester, formData: FormData) => {
    ...
    try {
      ...
      // 1. Create a profile for the newly approved tenant
      ...
      // 2. Create the tenant record linked to the profile and site
      ...
      // 3. Mark the requester as approved to remove it from the pending list
      const { error: reqErr } = await supabase.from('requesters').update({ status: 'approved' }).eq('id', requester.id);
  ...
  if (!currentUser.onboarded && currentUser.role === 'admin') return <Onboarding user={currentUser} onComplete={setCurrentUser} />;
    ...
    <Layout activeTab={activeTab} onTabChange={setActiveTab} dbStatus={dbStatus === 'error' ? 'error' : (dbStatus === 'connected' ? 'connected' : 'connecting')} tabConfigs={tabConfigs} orgName={organization?.name}>
            ...
            {requesters.map(req => (
              <div key={req.id} className="bg-white p-8 rounded-[40px] border border-slate-100 shadow-sm flex flex-col justify-between hover:border-blue-200 transition-all">
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <div><p className="text-lg font-black text-slate-900">+{req.phone}</p><p className="text-xs font-bold text-slate-400 uppercase">WhatsApp Request</p></div>
                </div>
                <button onClick={() => setShowApproveModal(req)} className="w-full bg-slate-900 text-white py-4 rounded-2xl font-black flex items-center justify-center gap-2 hover:bg-slate-800 transition-all"><UserCheck size={20} /> Setup Resident</button>
              </div>
            ))}
            {requesters.length === 0 && <div className="col-span-full py-20 text-center bg-white rounded-[40px] border-2 border-dashed border-slate-100 font-bold text-slate-400">No pending approvals.</div>}
          </div>
        </div>
      )}
      ...
      {activeTab === 'sites' && (
        <div className="space-y-8 pb-20">
          <div className="flex justify-between items-end">
            <div><h2 className="text-3xl font-black text-slate-900">Sites</h2><p className="text-slate-500 font-medium">Property map</p></div>
            ...
            {sites.map(site => (
              <div key={site.id} className="bg-white p-8 rounded-[40px] border border-slate-100 shadow-sm">
                ...
                <div className="pt-8 border-t border-slate-50">
                  <button onClick={() => { setActiveSiteForBlock(site.id); setShowAddBlock(true); }} className="text-xs font-bold text-blue-600 flex items-center gap-1 mb-4"><Plus size={14} /> Add Unit/Block</button>
      ...
      {showAddSR && (
        <div className="fixed inset-0 z-[1000] bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-4">
          <form onSubmit={async (e) => { e.preventDefault(); const fd = new FormData(e.currentTarget); await handleAddItem('service_requests', { title: fd.get('title'), description: fd.get('description'), site_id: fd.get('site_id') || null, status: SRStatus.NEW, source: SRSource.WEB, created_at: new Date().toISOString() }, setSrs, () => setShowAddSR(false)); }} className="bg-white w-full max-w-lg rounded-[40px] p-10 shadow-2xl space-y-8">
            ...
            <div className="space-y-4">
              <input required name="title" className="w-full bg-slate-50 rounded-2xl p-5 outline-none font-bold" placeholder="Short summary of issue" />
              <textarea name="description" className="w-full bg-slate-50 rounded-2xl p-5 outline-none font-medium h-32" placeholder="Full details..." />
              <select name="site_id" className="w-full bg-slate-50 rounded-2xl p-5 outline-none font-bold">
                <option value="">Select Site (Optional)</option>
                {sites.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
              </select>
            </div>
            <button type="submit" disabled={isLoading} className="w-full bg-blue-600 text-white py-6 rounded-3xl font-black text-xl flex items-center justify-center gap-3 active:scale-95 transition-all shadow-xl shadow-blue-100">
      ...
      {showAddAsset && (
        <div className="fixed inset-0 z-[1000] bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-4">
          <form onSubmit={async (e) => { e.preventDefault(); const fd = new FormData(e.currentTarget); await handleAddItem('assets', { name: fd.get('name'), type: fd.get('type'), code: `AST-${Math.floor(Math.random()*9000)}`, site_id: fd.get('site_id'), status: Status.ACTIVE }, setAssets, () => setShowAddAsset(false)); }} className="bg-white w-full max-w-lg rounded-[40px] p-10 shadow-2xl space-y-8">
            ...
            <div className="space-y-4">
              <input required name="name" className="w-full bg-slate-50 rounded-2xl p-5 outline-none font-bold" placeholder="Asset Name (e.g. Lift A)" />
              <input name="type" className="w-full bg-slate-50 rounded-2xl p-5 outline-none font-bold" placeholder="Type (e.g. Mechanical)" />
            ...
            <button type="submit" disabled={isLoading} className="w-full bg-slate-900 text-white py-6 rounded-3xl font-black flex items-center justify-center gap-3">
              {isLoading ? <Loader2 className="animate-spin" /> : 'Register Asset'}
            </button>
          </form>
        </div>
      )}
      ...
      {showAddTenant && (
        <div className="fixed inset-0 z-[1000] bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-4">
          <form onSubmit={async (e) => { e.preventDefault(); const fd = new FormData(e.currentTarget); await handleAddItem('tenants', { name: fd.get('name'), phone: fd.get('phone'), site_id: fd.get('site_id'), status: Status.ACTIVE }, setTenants, () => setShowAddTenant(false)); }} className="bg-white w-full max-w-lg rounded-[40px] p-10 shadow-2xl space-y-8">
            <div className="flex justify-between items-center"><h3 className="text-3xl font-black text-slate-900">Add Tenant</h3><button type="button" onClick={() => setShowAddTenant(false)} className="p-2 bg-slate-100 rounded-full"><X size={24} /></button></div>
            <div className="space-y-4">
              <input required name="name" className="w-full bg-slate-50 rounded-2xl p-5 outline-none font-bold" placeholder="Full Name" />
              <input required name="phone" className="w-full bg-slate-50 rounded-2xl p-5 outline-none font-bold" placeholder="Phone Number (Digits only)" />
              <select required name="site_id" className="w-full bg-slate-50 rounded-2xl p-5 outline-none font-bold">
            ...
            <button type="submit" disabled={isLoading} className="w-full bg-slate-900 text-white py-6 rounded-3xl font-black flex items-center justify-center gap-3">
              {isLoading ? <Loader2 className="animate-spin" /> : 'Save Tenant'}
            </button>
          </form>
        </div>
      )}
      ...
      {showAddBlock && (
        <div className="fixed inset-0 z-[1000] bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-4">
          <form onSubmit={async (e) => { e.preventDefault(); const fd = new FormData(e.currentTarget); await handleAddItem('blocks', { name: fd.get('name'), site_id: activeSiteForBlock, type: BlockType.SPACE }, setBlocks, () => setShowAddBlock(false)); }} className="bg-white w-full max-w-md rounded-[40px] p-10 shadow-2xl space-y-8">
            <div className="flex justify-between items-center"><h3 className="text-2xl font-black text-slate-900">New Unit/Block</h3><button type="button" onClick={() => setShowAddBlock(false)} className="p-2 bg-slate-100 rounded-full"><X size={24} /></button></div>
            <input required name="name" className="w-full bg-slate-50 rounded-2xl p-5 outline-none font-bold" placeholder="Block/Room Name" />
            <button type="submit" disabled={isLoading} className="w-full bg-slate-900 text-white py-5 rounded-3xl font-black">
              {isLoading ? <Loader2 className="animate-spin mx-auto" /> : 'Create Unit'}
            </button>
          </form>
        </div>
      )}
      ...
      {showApproveModal && (
        <div className="fixed inset-0 z-[1000] bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-4">
          <form onSubmit={async (e) => { e.preventDefault(); const fd = new FormData(e.currentTarget); await handleApproveTenant(showApproveModal, fd); }} className="bg-white w-full max-w-lg rounded-[40px] p-10 shadow-2xl space-y-8">
            <div className="flex justify-between items-center"><h3 className="text-3xl font-black text-slate-900">Approve Access</h3><button type="button" onClick={() => setShowApproveModal(null)} className="p-2 bg-slate-100 rounded-full"><X size={24} /></button></div>
            <div className="space-y-4">
              <div className="p-4 bg-blue-50 text-blue-700 rounded-2xl font-bold flex items-center gap-3"><Phone size={20} /> +{showApproveModal.phone}</div>
              <input required name="name" className="w-full bg-slate-50 rounded-2xl p-5 outline-none font-bold" placeholder="Resident Display Name" />
              <select required name="site_id" className="w-full bg-slate-50 rounded-2xl p-5 outline-none font-bold">
                <option value="">Assign to Site...</option>
                {sites.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
              </select>
            </div>
            <button type="submit" disabled={isLoading} className="w-full bg-emerald-600 text-white py-6 rounded-3xl font-black text-xl flex items-center justify-center gap-3 active:scale-95 shadow-xl shadow-emerald-100">
              {isLoading ? <Loader2 className="animate-spin" /> : 'Confirm & Activate'}
            </button>
          </form>
        </div>
      )}
